...
void TApp::CreateDiskPageManager(){
   ...
   if (in.is_open()){
      ...
      PageManagerMTree = new stPlainDiskPageManager(fileName);
      in.close();
   }else{
      // Opened.
   ...
   if (in.is_open()){
      // Cannot open. Create it.
      PageManagerSlimMST = new stPlainDiskPageManager(fileName);
      in.close();
   }else{
      // Opened.
   ...
   //for Slim-Tree SlimMinMax
   strcpy(fileName, this->Prefix.c_str());
   strcat(fileName, "SlimMinMax");
   strcat(fileName, pageSizeString);
   strcat(fileName, ".dat");
   in.open(fileName, ios_base::in);
   // Try to open the file.
   if (in.is_open()){
      // Cannot open. Create it.
      PageManagerSlimDown = new stPlainDiskPageManager(fileName);
      in.close();
   }else{
      // Opened.
...
void TApp::PerformQueries(){
   ...
   ResultMeasureDiscrete * rMeasurekNNQ = new ResultMeasureDiscrete(2, false, this->QtdKNNQ);
   ResultMeasure * rMeasureRQ = new ResultMeasure(false, this->QtdRQ, true, 3);
   ...
   gnuTestRQ->SetTitle(GNUPlot::DISTFUNCTION, "RQ: colorHisto");
   gnuTestRQ->SetTitle(GNUPlot::DISKACCESS, "RQ: colorHisto");
   gnuTestRQ->SetTitle(GNUPlot::TOTALTIME, "RQ: colorHisto");
   ...
   gnuTestkNNQ->SetTitle(GNUPlot::DISTFUNCTION, "kNNQ: colorHisto");
   gnuTestkNNQ->SetTitle(GNUPlot::DISKACCESS, "kNNQ: colorHisto");
   gnuTestkNNQ->SetTitle(GNUPlot::TOTALTIME, "kNNQ: colorHisto");
