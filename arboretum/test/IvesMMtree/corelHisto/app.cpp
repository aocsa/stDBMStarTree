...
void TApp::LoadTree(char * fileName, MetricTree * metricTree, clock_t * totalTime){
   ...
   THisto * histograms=NULL, * histo=NULL;
   ...
   if (metricTree!=NULL){
      if (in.is_open()){
         ...
         if (metricTreeVP = dynamic_cast<myVPTree *> (metricTree)){
            //It is the vp-tree
            histograms = new THisto[this->NumCities];
         }
         ...
         while(in >> id){
            ...
            histo = new THisto(MAXSIZE, values);
            histo->SetTag(id);

            if (metricTreeVP){
               histograms[i++].Unserialize(histo->Serialize(), histo->GetSerializedSize());
            }
            else{
               start = clock();
               this->AddToTree(histo, metricTree);
               end = clock();
         ...
         if (metricTreeVP){
            //Adds in the VP-tree
            start = clock();
            MetricTreeVP->Add(histograms, this->NumCities);
            end = clock();
            totalTimeTemp += end-start;
  ...
  ifstream in(fileName);
   ...
   if (in.is_open()){
      ...
      while (in >> id){
         ...
         basicArray = new THisto(MAXSIZE, values);
         basicArray->SetTag(id);
         // Add it in vector.
         queryCities.insert(queryCities.end(), basicArray);
         contObj++;
      }//end while
   ...
   if (metricTree){
      ...
      for (i=0; i < rMeasure->getSize(); i++){
         ...
         for (j=0; j < queryCities.size(); j++){
            sample = new THisto(queryCities[j]->GetSize(), queryCities[j]->GetData());
            sample->SetTag(queryCities[j]->GetTag());            
            result = metricTree->RangeQuery(sample, rMeasure->GetK(i)*1.41);
            delete result;
            delete sample;
         }//end for
         end = clock();
