   ...
   if (metricTree!=NULL){
      if (in.is_open()){
         ...
         if (metricTreeVP = dynamic_cast<myVPTree *> (metricTree)){
            //It is the vp-tree
            cities = new TWord[this->NumCities];
         }

         while(in.getline(wordName, 200)){
            if (metricTreeVP){
               cities[i++] = *( new TWord(wordName));
            }
            else{
               start = clock();
               this->AddToTree(wordName, metricTree);
               end = clock();
         ...
         if (metricTreeVP){
//          for(int i=0; i< this->NumCities; i++) printf("%i/%i. %s\n",i,this->NumCities,cities[i].GetString());
            //Adds in the VP-tree
            start = clock();
...
void TApp::LoadVectorFromFile(char * fileName){
   ...
   if (in.is_open()){
      ...
      while(in.getline(wordName, 200)){
//         cout << "A";
         queryCities.insert(queryCities.end(), &TWord(wordName));
         contObj++;
      }//end while
   ...
   if (metricTree){
      ...
      for (i=0; i < rMeasure->getSize(); i++){
         ...
         for (j=0; j < queryCities.size(); j++){
            sample = new TWord(queryCities[j]->GetString());
            result = metricTree->RangeQuery(sample, i+1);
            delete result;
            delete sample;
         }//end for
         end = clock();
         ...
         // is divided for queryCities to get the average
...
void TApp::PerformNearestQuery(ResultMeasureDiscrete * rMeasure, MetricTree * metricTree,
   ...
   if (metricTree){
      ...
      for (i=0; i < rMeasure->getSize(); i++){
         ...
         for (j=0; j < queryCities.size(); j++){
            sample = new TWord(queryCities[j]->GetString());
            result = metricTree->NearestQuery(sample, i+1);
            delete result;
            delete sample;
         }//end for
         end = clock();
