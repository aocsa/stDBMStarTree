...
// measure the distance between MAXSIZE TPoint instances.
//
...
//---------------------------------------------------------------------------
/**
* This class abstracts points in a space. Each point is in 6D.
*
* <P>In addition to data manipulation methods (such as GetValues()
...
class TPoint{
   public:
      /**
      * Default constructor. It creates a point with values set to 0.
      ...
      */
      bool IsEqual(TPoint * obj){
         ...
         for (int i=0; i<MAXSIZE; i++){
            if (this->GetValue(i) != obj->GetValue(i))
               return false;
         }//end for
...
class TPointDistanceEvaluator : public stMetricEvaluatorStatistics{
   public:
      ...
      /**
      * Returns the distance between 2 points. This method is required by
