...
void TApp::Run(){
   ...
   LoadVectorFromFile("LBeach500.txt");
   // Lets do the queries
   PerformQueries();
   // Hold the screen.
...
void TApp::CreateDiskPageManager(){
   ...
   dmanSlim->Open("LBeach256.dat");
//   dmanSlim->Open("LBeach4k.dat");
...
void TApp::PerformQueries(){
   ...
   fileDistFunc = fopen("DistFuncLB256.plt", "w");
//   fileDistFunc = fopen("DistFuncLB4k.plt", "w");
   WriteHeader(fileDistFunc, "set ylabel \"# AVG Distance Function\"\n");

   fileDiscAccess = fopen("DiscAccessLB256.plt", "w");
//   fileDiscAccess = fopen("DiscAccessLB4k.plt", "w");
   WriteHeader(fileDiscAccess, "set ylabel \"# AVG Disc Access\"\n");

   fileTime = fopen("TimeLB256.plt", "w");
//   fileTime = fopen("TimeLB4k.plt", "w");
...
void TApp::WriteHeader(FILE * file, char * type){
   ...
   strcpy(strTemp,"set title \"LBeach: Nearest Queries");
   fwrite(strTemp, strlen(strTemp), 1, file);
   strcpy(strTemp,"\"\nset logscale xy\n");
