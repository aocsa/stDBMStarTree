...
void TApp::LoadTree(char * fileName){
   ...
   if (in.is_open()){
      ...
      while(in.getline(wordName, 200)){
         this->AddToTree(wordName);
         w++;
         if (w % 100 == 0){
            cout << '.';
         }//end if
...
void TApp::LoadVectorFromFile(char * fileName){
   ...
   if (in.is_open()){
      ...
      contObj = 0;
      while(in.getline(wordName, 200)){
         queryWords.insert(queryWords.end(), new TWord(wordName));
         contObj++;
      }//end while
...
void TApp::PerformQueries(){
   ...
   ResultMeasureDiscrete * rMeasureKOrRange = new ResultMeasureDiscrete(this->DatasetSize);
   ResultMeasureDiscrete * rMeasureSimpleKOrRange = new ResultMeasureDiscrete(this->DatasetSize);
...
//------------------------------------------------------------------------------
void TApp::PerformSimpleKOrRangeQuery(ResultMeasureDiscrete * rMeasure, int i){
   myResult * result;
   TWord * sample;
   clock_t start, end;
   int k, j;
   ...
   rMeasure->SetDiscAccess(i, (double) PageManagerSlim->GetReadCount() / (double) queryWords.size());
   // is divided for queryWords to get the everage
   rMeasure->SetDistanceFunction(i,
      (double) MetricTreeSlim->GetMetricEvaluator()->GetDistanceCount() / (double) queryWords.size());
...
//------------------------------------------------------------------------------
void TApp::PerformKOrRangeQuery(ResultMeasureDiscrete * rMeasure, int i){
   myResult * result;
   TWord * sample;
   clock_t start, end;
   int k, j;
   ...
   for (j=0; j < queryWords.size(); j++){
      sample = new TWord(queryWords[j]->GetString());
      result = MetricTreeSlim->KOrRangeQuery(sample,
                             radiusQueryWords[j], k);
      delete result;
      delete sample;
   }//end for
...
//------------------------------------------------------------------------------
void TApp::WriteLog(ResultMeasureDiscrete * result, char * name){
...
void TApp::WriteHeader(FILE * file, char * type){
   ...
   // Title
   strcpy(strTemp,"set title \"Brazilian Cities - Disjuntive Complex Queries ");
   strcat(strTemp,"D= ");
   fwrite(strTemp, strlen(strTemp), 1, file);
...
void TApp::SetRadiusFromObjects(ResultMeasureDiscrete * rMeasureSlim, int i){
   ...
   oneThird = (unsigned int) floor(queryWords.size() / this->Third);
   //clear before insert new radium values
   radiusQueryWords.clear();
   // set the number of objects.
   numberOfObjects = MetricTreeSlim->GetNumberOfObjects() * rMeasureSlim->GetK(i);
   ...
   for (j=0; j < queryWords.size(); j++){
      ...
      radiusQueryWords.insert(radiusQueryWords.end(), radius);
      delete result;
      delete sample;
  }//end for
