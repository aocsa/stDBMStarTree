...
void TApp::PerformSimpleKOrRangeQuery(ResultMeasure * rMeasure, int i){
   ...
   for (j=0; j < queryMetricHistogram.size(); j++){
      ...
      result = MetricTreeSlim->SimpleKOrRangeQuery(sample,
                                   ...
                                   radiusQueryMetricHistogram[j], k);
      delete result;
      delete sample;
   }//end for
   end = clock();
   ...
   rMeasure->SetDiscAccess(i, (double) PageManagerSlim->GetReadCount() / (double) queryMetricHistogram.size());
   // is divided for queryCities to get the everage
   rMeasure->SetDistanceFunction(i,
      (double) MetricTreeSlim->GetMetricEvaluator()->GetDistanceCount() / (double) queryMetricHistogram.size());
...
void TApp::PerformKOrRangeQuery(ResultMeasure * rMeasure, int i){
   ...
   for (j=0; j < queryMetricHistogram.size(); j++){
      sample = new tMetricHistogram(*queryMetricHistogram[j]);
      result = MetricTreeSlim->KOrRangeQuery(sample, radiusQueryMetricHistogram[j], k);
      delete result;
      delete sample;
   }//end for
   end = clock();
...
void TApp::WriteHeader(FILE * file, char * type){
   ...
   // Title
   strcpy(strTemp,"set title \"Cities - Disjuntive Complex Queries ");
   strcat(strTemp,"D= ");
   fwrite(strTemp, strlen(strTemp), 1, file);
...
void TApp::SetRadiusFromObjects(ResultMeasure * rMeasure, int i){
   ...
   oneThird = (unsigned int) floor(queryMetricHistogram.size() / this->Third);
   //clear before insert new radium values
   radiusQueryMetricHistogram.clear();
   ...
   for (j=0; j < queryMetricHistogram.size(); j++){
      ...
      radiusQueryMetricHistogram.insert(radiusQueryMetricHistogram.end(), radius);
      delete result;
      delete sample;
   }//end for
