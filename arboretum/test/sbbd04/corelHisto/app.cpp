...
void TApp::LoadVectorFromFile(char * fileName){
   ...
   if (in.is_open()){
      ...
      while (in >> id){
         ...
         basicArray->SetTag(id);
         // Add it in vector.
         queryPoints.insert(queryPoints.end(), basicArray);
         contObj++;
      }//end while
...
void TApp::PerformSimpleKOrRangeQuery(ResultMeasure * rMeasure, int i){
   ...
   for (j=0; j < queryPoints.size(); j++){
      sample = new myBasicArrayObject(queryPoints[j]->GetSize(), queryPoints[j]->GetData());
      sample->SetTag(queryPoints[j]->GetTag());
      result = MetricTreeSlim->SimpleKOrRangeQuery(sample, this->FractalDimension,
