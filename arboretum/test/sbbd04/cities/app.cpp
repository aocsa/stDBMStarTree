...
void TApp::LoadTree(char * fileName){
   ...
   if (in.is_open()){
      ...
      while(in.getline(cityName, 200, '\t')){
         ...
         in >> dLong;
         in.ignore();
         this->AddToTree(cityName, dLat, dLong);
         w++;
         if (w % 10 == 0){
            cout << '.';
         }//end if
...
void TApp::PerformSimpleKOrRangeQuery(ResultMeasure * rMeasure, int i){
   ...
   rMeasure->SetDiscAccess(i, (double) PageManagerSlim->GetReadCount() / (double) queryCities.size());
   // is divided for queryCities to get the everage
   rMeasure->SetDistanceFunction(i,
      (double) MetricTreeSlim->GetMetricEvaluator()->GetDistanceCount() / (double) queryCities.size());
...
void TApp::PerformKOrRangeQuery(ResultMeasure * rMeasure, int i){
   ...
   for (j=0; j < queryCities.size(); j++){
      sample = new TCity(queryCities[j]->GetName().c_str(),
                         queryCities[j]->GetLatitude(),
                         queryCities[j]->GetLongitude());
      result = MetricTreeSlim->KOrRangeQuery(sample, radiusQueryCities[j], k);
      delete result;
      delete sample;
   }//end for
   end = clock();
...
void TApp::SetRadiusFromObjects(ResultMeasure * rMeasure, int i){
   ...
   oneThird = (unsigned int) floor(queryCities.size() / this->Third);
   ...
   for (j=0; j < queryCities.size(); j++){
      ...
      radiusQueryCities.insert(radiusQueryCities.end(), radius);
      delete result;
      delete sample;
   }//end for
