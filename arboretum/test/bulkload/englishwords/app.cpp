...
void TApp::LoadTreeBulk(char * fileName){
   ...
   TWord ** objects = new TWord * [MAXOBJ];
   ...
   long w = 0;
      ...
      if (in.is_open()){
         ...
         while (in.getline(wordName, 30)){
            memset(wordName + strlen(wordName), 0, 30 - strlen(wordName));
            wordName[30] = '\0';
            objects[w] = new TWord(wordName);
            w++;
            if (w % 100 == 0){
               cout << '.';
            }//end if
...
void TApp::LoadTreeNormal(char * fileName){
   ...
   long w = 0;
      ...
      if (in.is_open()){
         ...
         //while ((in.getline(wordName, 30)) && (w < 1000)){
         while (in.getline(wordName, 30)){
            memset(wordName + strlen(wordName), 0, 30 - strlen(wordName));
            wordName[30] = '\0';
            this->AddToTree(wordName, MTreeNormal);
            w++;
            if (w % 100 == 0){
               cout << '.';
            }//end if
   ...
   long w = 0;
      ...
      if (in.is_open()){
         ...
         while (in.getline(wordName, 30)){
            memset(wordName + strlen(wordName), 0, 30 - strlen(wordName));
            wordName[30] = '\0';
            this->AddToTree(wordName, SlimTreeNormal);
            w++;
            if (w % 100 == 0){
               cout << '.';
            }//end if
...
void TApp::LoadVectorFromFile(char * fileName){
   ...
   if (in.is_open()){
      ...
      while (in.getline(wordName, 30)){
         memset(wordName + strlen(wordName), 0, 30 - strlen(wordName));
         wordName[30] = '\0';
         queryObjects.insert(queryObjects.end(), new TWord(wordName));
      }//end while
...
void TApp::PerformQueries(){
   ...
   fileNameDF = string("EiRQDist") + pageSizeString + ".plt";
   fileNameDA = string("EiRQDisc") + pageSizeString + ".plt";
   fileNameTT = string("EiRQTime") + pageSizeString + ".plt";
   GNUPlot * gnuTestRQ = new GNUPlot(fileNameDA, fileNameDF, fileNameTT);

   fileNameDF = string("EikNNQDist") + pageSizeString + ".plt";
   fileNameDA = string("EikNNQDisc") + pageSizeString + ".plt";
   fileNameTT = string("EikNNQTime") + pageSizeString + ".plt";
   GNUPlot * gnuTestkNNQ = new GNUPlot(fileNameDA, fileNameDF, fileNameTT);

   gnuTestRQ->SetTitle(GNUPlot::DISTFUNCTION, "RQ: EigenFaces");
   gnuTestRQ->SetTitle(GNUPlot::DISKACCESS, "RQ: EigenFaces");
   gnuTestRQ->SetTitle(GNUPlot::TOTALTIME, "RQ: EigenFaces");
   ...
   gnuTestkNNQ->SetTitle(GNUPlot::DISTFUNCTION, "kNNQ: EigenFaces");
   gnuTestkNNQ->SetTitle(GNUPlot::DISKACCESS, "kNNQ: EigenFaces");
   gnuTestkNNQ->SetTitle(GNUPlot::TOTALTIME, "kNNQ: EigenFaces");
