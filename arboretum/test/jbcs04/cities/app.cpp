...
void TApp::NearestAndRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j < queryCities.size(); j++){
         ...
         result = MetricTreeSlim->KAndRangeQuery(sample, radiusQueryCities[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      rMeasureSlim->setDiscAccess(i, (double )PageManagerSlim->GetReadCount() / (double )queryCities.size());
      rMeasureSlim->setDistanceFunction(i, (double )MetricTreeSlim->GetMetricEvaluator()->GetDistanceCount() /
      												 (double )queryCities.size());
      ...
      for (j=0; j < queryCities.size(); j++){
         sample = new TCity(queryCities[j]->GetName().c_str(),
                            queryCities[j]->GetLatitude(),
                            queryCities[j]->GetLongitude());
         result = MetricTreeDummy->KAndRangeQuery(sample, radiusQueryCities[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      rMeasureDummy->setDiscAccess(i, (double )PageManagerDummy->GetReadCount() / (double )queryCities.size());
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j<queryCities.size(); j++){
         sample = new TCity(queryCities[j]->GetName().c_str(),
                            queryCities[j]->GetLatitude(),
                            queryCities[j]->GetLongitude());
         result = MetricTreeSlim->KOrRangeQuery(sample, radiusQueryCities[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      for (j=0; j<queryCities.size(); j++){
         sample = new TCity(queryCities[j]->GetName().c_str(),
                            queryCities[j]->GetLatitude(),
                            queryCities[j]->GetLongitude());
         result = MetricTreeDummy->KOrRangeQuery(sample, radiusQueryCities[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j < queryCities.size(); j++){
         sample = new TCity(queryCities[j]->GetName().c_str(),
                            queryCities[j]->GetLatitude(),
                            queryCities[j]->GetLongitude());
         result1 = MetricTreeSlim->RangeQuery(sample, radiusQueryCities[i][j]);
      ...
      for (j=0; j < queryCities.size(); j++){
         sample = new TCity(queryCities[j]->GetName().c_str(),
                            queryCities[j]->GetLatitude(),
                            queryCities[j]->GetLongitude());
         result1 = MetricTreeDummy->RangeQuery(sample, radiusQueryCities[i][j]);
...
void TApp::SetRadiusFromObjects(resultMeasure * rMeasureSlim){
   ...
   for (i=0; i < this->QuantidadeRange; i++){
      ...
      for (j=0; j < queryCities.size(); j++){
         ...
         // set the radius.
         radiusQueryCities[i].insert(radiusQueryCities[i].end(), result->GetMaximumDistance());
         delete result;
         delete sample;
      }//end for
      cout << " " << i;
...
void TApp::WriteHeader(FILE * file, char * type, bool and, double radius){
   ...
   strcpy(strTemp,"set title \"Cities dataset:");
   fwrite(strTemp, strlen(strTemp), 1, file);
   strcpy(strTemp,"  page=");
   fwrite(strTemp, strlen(strTemp), 1, file);
