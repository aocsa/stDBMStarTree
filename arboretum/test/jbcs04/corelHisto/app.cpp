...
void TApp::Done(){
   ...
   for (int i=0; i < queryHisto.size(); i++){
      delete (queryHisto.at(i));
   }//end for
}//end TApp::Done
   ...
   if (metricTree!=NULL){
      if ((file = fopen(fileName, "r")) == NULL){
         ...
         do{
            ...
            basicArray->SetTag(id);
            // Add it in Slim-Tree.
            AddToTree(basicArray, metricTree);
            delete basicArray;
            // read the id.
            fscanf(file, "%d", &id);
...
void TApp::LoadVectorFromFile(char * fileName){
   ...
   if ((file = fopen(fileName, "r")) == NULL){
      ...
      do{
         ...
         basicArray->SetTag(id);
         // Add it in vector.
         queryHisto.insert(queryHisto.end(), basicArray);
         contObj++;
         // read the id.
         fscanf(file, "%d", &id);
      }while(!feof(file));//end while
...
void TApp::NearestAndRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      start = clock();
      for (j=0; j < queryHisto.size(); j++){
         sample = new myBasicArrayObject(queryHisto[j]->GetSize(), queryHisto[j]->GetData());
         sample->SetTag(queryHisto[j]->GetTag());
         result = MetricTreeSlim->KAndRangeQuery(sample, radiusQueryHisto[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      rMeasureSlim->setDiscAccess(i, (double )PageManagerSlim->GetReadCount() / (double )queryHisto.size());
      rMeasureSlim->setDistanceFunction(i, (double )MetricTreeSlim->GetMetricEvaluator()->GetDistanceCount() /
      												 (double )queryHisto.size());

      start = clock();
      for (j=0; j < queryHisto.size(); j++){
         sample = new myBasicArrayObject(queryHisto[j]->GetSize(), queryHisto[j]->GetData());
         sample->SetTag(queryHisto[j]->GetTag());
         result = MetricTreeDummy->KAndRangeQuery(sample, radiusQueryHisto[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      rMeasureDummy->setDiscAccess(i, (double )PageManagerDummy->GetReadCount() / (double )queryHisto.size());
...
void TApp::NearestOrRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j<queryHisto.size(); j++){
         sample = new myBasicArrayObject(queryHisto[j]->GetSize(), queryHisto[j]->GetData());
         sample->SetTag(queryHisto[j]->GetTag());
         result = MetricTreeSlim->KOrRangeQuery(sample, radiusQueryHisto[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      for (j=0; j<queryHisto.size(); j++){
         sample = new myBasicArrayObject(queryHisto[j]->GetSize(), queryHisto[j]->GetData());
         sample->SetTag(queryHisto[j]->GetTag());
         result = MetricTreeDummy->KOrRangeQuery(sample, radiusQueryHisto[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
...
void TApp::NearestIntersectionRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j < queryHisto.size(); j++){
         sample = new myBasicArrayObject(queryHisto[j]->GetSize(), queryHisto[j]->GetData());
         sample->SetTag(queryHisto[j]->GetTag());
         result1 = MetricTreeSlim->RangeQuery(sample, radiusQueryHisto[i][j]);
      ...
      for (j=0; j < queryHisto.size(); j++){
         sample = new myBasicArrayObject(queryHisto[j]->GetSize(), queryHisto[j]->GetData());
         sample->SetTag(queryHisto[j]->GetTag());
         result1 = MetricTreeDummy->RangeQuery(sample, radiusQueryHisto[i][j]);
...
void TApp::SetRadiusFromObjects(resultMeasure * rMeasureSlim){
   ...
   for (i=0; i < this->QuantidadeRange; i++){
      ...
      for (j=0; j < queryHisto.size(); j++){
         ...
         // set the radius.
         radiusQueryHisto[i].insert(radiusQueryHisto[i].end(), result->GetMaximumDistance());
         delete result;
         delete sample;
      }//end for
      cout << " " << i;
...
void TApp::WriteHeader(FILE * file, char * type, bool and, double radius){
   ...
   strcpy(strTemp,"set title \"Images dataset:");
   fwrite(strTemp, strlen(strTemp), 1, file);
   strcpy(strTemp,"  page=");
   fwrite(strTemp, strlen(strTemp), 1, file);
