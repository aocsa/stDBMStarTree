...
void TApp::Done(){
   ...
   for (int i=0; i < queryMetricHistogram.size(); i++){
      delete (queryMetricHistogram.at(i));
   }//end for
}//end TApp::Done
...
void TApp::LoadTree(char * fileName, MetricTree * metricTree){
   ...
   if (metricTree!=NULL){
      ...
      do{
         ...
         AddToTree(bins, size, metricTree);
         delete [] bins;
         fscanf(file, "%d", &size);
...
void TApp::LoadVectorFromFile(char * fileName){
   ...
   do{
      ...
      queryMetricHistogram.insert(queryMetricHistogram.end(),
...
void TApp::NearestAndRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      start = clock();
      for (j=0; j < queryMetricHistogram.size(); j++){
         sample = new tMetricHistogram(*queryMetricHistogram[j]);
         result = MetricTreeSlim->KAndRangeQuery(sample, radiusQueryMetricHistogram[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      rMeasureSlim->setDiscAccess(i, (double )PageManagerSlim->GetReadCount() / (double )queryMetricHistogram.size());
      rMeasureSlim->setDistanceFunction(i, (double )MetricTreeSlim->GetMetricEvaluator()->GetDistanceCount() /
      												 (double )queryMetricHistogram.size());

      start = clock();
      for (j=0; j < queryMetricHistogram.size(); j++){
         sample = new tMetricHistogram(*queryMetricHistogram[j]);
         result = MetricTreeDummy->KAndRangeQuery(sample, radiusQueryMetricHistogram[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      rMeasureDummy->setDiscAccess(i, (double )PageManagerDummy->GetReadCount() / (double )queryMetricHistogram.size());
...
void TApp::NearestOrRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j<queryMetricHistogram.size(); j++){
         sample = new tMetricHistogram(*queryMetricHistogram[j]);
         result = MetricTreeSlim->KOrRangeQuery(sample, radiusQueryMetricHistogram[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      for (j=0; j<queryMetricHistogram.size(); j++){
         sample = new tMetricHistogram(*queryMetricHistogram[j]);
         result = MetricTreeDummy->KOrRangeQuery(sample, radiusQueryMetricHistogram[i][j], numberOfObjects, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
...
void TApp::NearestIntersectionRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j < queryMetricHistogram.size(); j++){
         sample = new tMetricHistogram(*queryMetricHistogram[j]);
         result1 = MetricTreeSlim->RangeQuery(sample, radiusQueryMetricHistogram[i][j]);
      ...
      for (j=0; j < queryMetricHistogram.size(); j++){
         sample = new tMetricHistogram(*queryMetricHistogram[j]);
         result1 = MetricTreeDummy->RangeQuery(sample, radiusQueryMetricHistogram[i][j]);
...
void TApp::SetRadiusFromObjects(resultMeasure * rMeasureSlim){
   ...
   for (i=0; i < this->QuantidadeRange; i++){
      radiusQueryMetricHistogram[i].clear();
      ...
      for (j=0; j < queryMetricHistogram.size(); j++){
         ...
         // set the radius.
         radiusQueryMetricHistogram[i].insert(radiusQueryMetricHistogram[i].end(), result->GetMaximumDistance());
         delete result;
         delete sample;
      }//end for
      cout << " " << i;
...
void TApp::WriteHeader(FILE * file, char * type, bool and, double radius){
   ...
   strcpy(strTemp,"set title \"MetricHisto dataset:");
   fwrite(strTemp, strlen(strTemp), 1, file);
   strcpy(strTemp,"  page=");
   fwrite(strTemp, strlen(strTemp), 1, file);
