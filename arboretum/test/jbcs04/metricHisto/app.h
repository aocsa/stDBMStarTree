...
class TApp{
   ...
   private:
      ...
      */
      vector <tMetricHistogram *> queryMetricHistogram;

      /**
      * Vector for holding the radius of the query objects.
      */
      vector <double> radiusQueryMetricHistogram[13];
      ...
      *
      */
      void AddToTree(tMetricHistogramBin * bins, int size, MetricTree * metricTree){
         tMetricHistogram * metricHisto = new tMetricHistogram(size, bins);
         metricTree->Add(metricHisto);
         delete metricHisto;
      }//end AddToTree
