...
void TApp::PerformQueries(){
   ...
   for (idx=0; idx<this->QuantidadeK; idx++){  // raio comecando de 0.01 = 1% (valor absoluto)
      ...
      sprintf(fileName+strlen(fileName), "%d", idx+1);
      strcat(fileName, "_");
      strcat(fileName, pageSizeString);
      strcat(fileName, ".plt");
      ...
      cout << "\nTests with radius: " << idx+1 << "\n\n";

      cout << "\nStarting Statistics for kAndRange Query.... ";
      NearestAndRange(idx+1, rMeasureSlimKAndR, rMeasureDummyKAndR);
      cout << " Ok\n";

      cout << "\nStarting Statistics for NearestIntersectionRange Query.... ";
      NearestIntersectionRange(idx+1, rMeasureSlimInter, rMeasureDummyInter);
      cout << " Ok\n";

      cout << "\nStarting Statistics for kOrRange Query.... ";
      NearestOrRange(idx+1, rMeasureSlimKOrR, rMeasureDummyKOrR);
      cout << " Ok\n";

      cout << "\nStarting Statistics for NearestUnionRange Query.... ";
      NearestUnionRange(idx+1, rMeasureSlimUnion, rMeasureDummyUnion);
      cout << " Ok\n";
...
void TApp::NearestAndRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j < queryWords.size(); j++){
         sample = new TWord(queryWords[j]->GetString());

         result = MetricTreeSlim->KAndRangeQuery(sample, i+1, numberOfK, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      rMeasureSlim->setDiscAccess(i, (double )PageManagerSlim->GetReadCount() / (double )queryWords.size());
      ...
      for (j=0; j < queryWords.size(); j++){
         ...
         result = MetricTreeDummy->KAndRangeQuery(sample, i+1, numberOfK, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      rMeasureDummy->setDiscAccess(i, (double )PageManagerDummy->GetReadCount() / (double )queryWords.size());
...
void TApp::NearestOrRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j < queryWords.size(); j++){
         ...
         result = MetricTreeSlim->KOrRangeQuery(sample, i+1, numberOfK, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
      ...
      for (j=0; j < queryWords.size(); j++){
         ...
         result = MetricTreeDummy->KOrRangeQuery(sample, i+1, numberOfK, this->TieList);
         delete result;
         delete sample;
      }//end for
      end = clock();
...
void TApp::NearestIntersectionRange(int idx, resultMeasure * rMeasureSlim, resultMeasure * rMeasureDummy){
   ...
   for (i=0; i < rMeasureSlim->getSize(); i++){
      ...
      for (j=0; j < queryWords.size(); j++){
         ...
         result1 = MetricTreeSlim->RangeQuery(sample, i+1);
         result2 = MetricTreeSlim->NearestQuery(sample, numberOfK, this->TieList);
      ...
      for (j=0; j < queryWords.size(); j++){
         ...
         result1 = MetricTreeDummy->RangeQuery(sample, i+1);
         result2 = MetricTreeDummy->NearestQuery(sample, numberOfK, this->TieList);
         //result1 intersection result2
         finalResult = new myResult();
         finalResult->Intersection(result1, result2);
...
void TApp::WriteHeader(FILE * file, char * type, bool and, int radius){
   ...
   strcpy(strTemp,"set title \"Words dataset:");
   fwrite(strTemp, strlen(strTemp), 1, file);
   strcpy(strTemp,"  page=");
   fwrite(strTemp, strlen(strTemp), 1, file);
